...
class ConvertNet(nn.Module):
    ...
    def __init__(self, args, inp_dim, dropout, conv_type):
        ...
        if conv_type in ['TransformerHEA', 'HEA']:
            ...
            if conv_type == 'MultilevelMemoryHEA':
                ...
                transition_feature_dim = getattr(args, 'transition_feature_dim', 9)
                analysis_dim = getattr(args, 'analysis_dim_after_reduction', inp_dim)

                analysis_as_expert = getattr(args, 'analysis_as_expert', False)
                fusion_mode = getattr(args, 'fusion_mode', 'film')
                self.sub_module = MultilevelMemoryHEA(
                    ...
                    enhanced_gating=enhanced_gating,
        ...
        print(f"convert module: {conv_type}")
    ...
    def forward(self, x_list, ls_ratios=None, multilevel_memory_data=None):
        if self.type in ['TransformerHEA', 'HEA']:
            ...
            # TransformerHEA和HEA返回task_outputs列表，需要拼接
...
class MultilevelMemoryHEA(nn.Module):
    """
    多级记忆混合专家适配器 (Multilevel Memory Hybrid Expert Adaptor)

    支持三种模式:
    - legacy: 旧版伪三级投影 + MultiHeadAttention (兼容旧实验)
    - reflection: Agent式反思调制 (MTR, Memory Transition Reflection)
      使用 FiLM 对 VNuser/VNcourse 进行条件调制
    - analysis_as_expert: 分析向量作为 MoE 第三专家输入
      直接将分析向量投影到 inp_dim，作为第三条任务路径加入 PLE 专家网络
      门控网络自适应决定是否采纳分析视角
    """
    def __init__(self, ple_arch, inp_dim, dropout,
                 ...
                 enhanced_gating=False, reflection_mode=False, transition_feature_dim=9,
                 analysis_dim=None, analysis_as_expert=False, fusion_mode='film'):
        ...
        assert fusion_mode in ('film', 'concat', 'xattn'), f"unsupported fusion_mode={fusion_mode}"
        self.fusion_mode = fusion_mode

        if analysis_dim is None:
            analysis_dim = inp_dim
        self.analysis_dim = analysis_dim

        if self.reflection_mode:
            if self.fusion_mode == 'film':
                # ── MTR: Memory Transition Reflection (v1-lite, lightweight FiLM) ──
                film_hidden = analysis_dim * 2
                self.film_user = nn.Sequential(
                    nn.Linear(analysis_dim, film_hidden),
                    nn.GELU(),
                    nn.Dropout(dropout),
                    nn.Linear(film_hidden, inp_dim * 2),
                )
                self.film_course = nn.Sequential(
                    nn.Linear(analysis_dim, film_hidden),
                    nn.GELU(),
                    nn.Dropout(dropout),
                    nn.Linear(film_hidden, inp_dim * 2),
                )
                print(f"MultilevelMemoryHEA: MTR反思调制模式 (论文FiLM)")
                print(f"  FiLM: analysis_dim={analysis_dim} → hidden={film_hidden} → inp_dim={inp_dim}")
            elif self.fusion_mode == 'concat':
                # ── B1: Concat + Linear 融合（消融 FiLM 的最简基线）──
                proj_hidden = inp_dim * 2
                self.proj_cat_user = nn.Sequential(
                    nn.Linear(inp_dim + analysis_dim, proj_hidden),
                    nn.GELU(),
                    nn.Dropout(dropout),
                    nn.Linear(proj_hidden, inp_dim),
                )
                self.proj_cat_course = nn.Sequential(
                    nn.Linear(inp_dim + analysis_dim, proj_hidden),
                    nn.GELU(),
                    nn.Dropout(dropout),
                    nn.Linear(proj_hidden, inp_dim),
                )
                print(f"MultilevelMemoryHEA: Concat 融合模式 (B1)")
                print(f"  Concat+Linear: {inp_dim + analysis_dim} → {proj_hidden} → {inp_dim}")
            elif self.fusion_mode == 'xattn':
                # ── B2: Cross-Attention 融合 (Q=v_a, K=V=v_u/v_c) ──
                if analysis_dim != inp_dim:
                    # 先把 analysis 投影到 inp_dim 使 MHA embed_dim 一致
                    self.xattn_analysis_proj = nn.Linear(analysis_dim, inp_dim)
                else:
                    self.xattn_analysis_proj = nn.Identity()
                xattn_heads = 8 if inp_dim % 8 == 0 else 4 if inp_dim % 4 == 0 else 1
                self.xattn_user = nn.MultiheadAttention(
                    embed_dim=inp_dim, num_heads=xattn_heads,
                    dropout=dropout, batch_first=True
                )
                self.xattn_course = nn.MultiheadAttention(
                    embed_dim=inp_dim, num_heads=xattn_heads,
                    dropout=dropout, batch_first=True
                )
                self.xattn_norm_user = nn.LayerNorm(inp_dim)
                self.xattn_norm_course = nn.LayerNorm(inp_dim)
                print(f"MultilevelMemoryHEA: Cross-Attention 融合模式 (B2)")
                print(f"  MHA: embed_dim={inp_dim}, heads={xattn_heads}, Q=v_a, K=V=v_u/v_c")
        elif self.enable_multilevel_attention:
            # ── Legacy: 旧版伪三级投影 (兼容) ──
            # 使用 analysis_dim 作为投影输入维度（兼容 BGE-M3 降维后的向量）
            self.W_sensory = nn.Linear(analysis_dim, inp_dim)
            self.W_working = nn.Linear(analysis_dim, inp_dim)
            self.W_longterm = nn.Linear(analysis_dim, inp_dim)

            self.sensory_attention = MultiHeadAttention(
                input_dim=inp_dim, num_heads=sensory_attn_heads,
                dropout_rate=dropout, use_residual=True, layer_norm=True, align_to="input"
            )
            self.working_attention = MultiHeadAttention(
                input_dim=inp_dim, num_heads=working_attn_heads,
                dropout_rate=dropout, use_residual=True, layer_norm=True, align_to="input"
            )

            gating_input_dim = 3 + (6 if enhanced_gating else 0)
            self.memory_weight_adaptor = nn.Sequential(
                nn.Linear(gating_input_dim, inp_dim // 4),
                nn.ReLU(),
                nn.Dropout(dropout),
                nn.Linear(inp_dim // 4, 3),
                nn.Softmax(dim=-1)
            )
            print(f"MultilevelMemoryHEA: Legacy注意力模式 (analysis_dim={analysis_dim}→inp_dim={inp_dim})")

        # ── analysis_as_expert: 分析向量→第三专家路径 ──
        # 此模式与 reflection_mode 互斥（reflection 已消耗了 analysis 向量）
        if self.analysis_as_expert and not self.reflection_mode:
            self.analysis_proj = nn.Sequential(
                nn.Linear(analysis_dim, inp_dim),
                nn.LayerNorm(inp_dim),
                nn.GELU(),
                nn.Dropout(dropout)
            )
            # 第三路任务特定专家（analysis 专家）
            self.analysis_expt_net = nn.ModuleList([
                nn.ModuleList([
                    MLP(expt_arch, inp_dim, dropout) for _ in range(spcf_expt_num)
                ]) for _ in range(task_num)
            ])
            # 三路门控（user/course/analysis）
            total_experts = share_expt_num + spcf_expt_num * 2  # 增加 analysis 专家路
            print(f"MultilevelMemoryHEA: analysis_as_expert 模式 (第三专家路径)")
            print(f"  analysis_dim={analysis_dim} → proj → inp_dim={inp_dim}")
            print(f"  total_experts={total_experts} (share={share_expt_num} + spcf×3={spcf_expt_num*3})")
        else:
            self.analysis_proj = None
            self.analysis_expt_net = None
            total_experts = share_expt_num + spcf_expt_num
        ...
        # Equation (19): gate input is semantic || distribution || transition.
        gate_inp_dim = inp_dim
        gate_inp_dim += (6 if enhanced_gating else 0)
        gate_inp_dim += (transition_feature_dim if reflection_mode else 0)

        gate_hidden_dim = max(64, total_experts * 2)
        self.gate_net = nn.ModuleList([
            nn.Sequential(
                nn.Linear(gate_inp_dim, gate_hidden_dim),
                nn.ReLU(),
                nn.Dropout(dropout),
                nn.Linear(gate_hidden_dim, total_experts),
                nn.Softmax(dim=-1)
            ) for _ in range(task_num)
        ])
        self._total_experts = total_experts
        self._share_expt_num = share_expt_num
        self._spcf_expt_num = spcf_expt_num

        print(
            f"  门控输入维度: {gate_inp_dim} "
            f"(semantic={inp_dim} + dist={6 if enhanced_gating else 0} "
            f"+ transition={transition_feature_dim if reflection_mode else 0})"
        )
        print(f"MultilevelMemoryHEA: 专家网络 share={share_expt_num} spcf={spcf_expt_num} tasks={task_num}")

    def forward(self, x_list, multilevel_memory_data=None):
        """
        Args:
            x_list: [VNuser, VNcourse] 或 [VNuser, VNcourse, VNanalysis]
            multilevel_memory_data: 多级记忆数据字典
        """
        # ── 预提取 analysis 向量（在任何路径截断前保存） ──
        raw_analysis_vec = x_list[2] if len(x_list) >= 3 else None

        if self.reflection_mode and multilevel_memory_data is not None and len(x_list) >= 3:
            # ── MTR v1: Memory Transition Reflection ──
            VNuser = x_list[0]
            VNcourse = x_list[1]
            VNanalysis = x_list[2]
            VNanalysis = torch.nn.functional.normalize(VNanalysis, p=2, dim=-1)
            device = VNuser.device
            batch_size = VNuser.size(0)

            if self.fusion_mode == 'film':
                user_params = self.film_user(VNanalysis)
                user_scale, user_shift = user_params.chunk(2, dim=-1)

                course_params = self.film_course(VNanalysis)
                course_scale, course_shift = course_params.chunk(2, dim=-1)

                V_user_enh = user_scale * VNuser + user_shift
                V_course_enh = course_scale * VNcourse + course_shift
            elif self.fusion_mode == 'concat':
                V_user_enh = self.proj_cat_user(torch.cat([VNuser, VNanalysis], dim=-1))
                V_course_enh = self.proj_cat_course(torch.cat([VNcourse, VNanalysis], dim=-1))
            elif self.fusion_mode == 'xattn':
                q = self.xattn_analysis_proj(VNanalysis).unsqueeze(1)
                ku = VNuser.unsqueeze(1)
                kc = VNcourse.unsqueeze(1)
                out_u, _ = self.xattn_user(q, ku, ku)
                out_c, _ = self.xattn_course(q, kc, kc)
                V_user_enh = self.xattn_norm_user(VNuser + out_u.squeeze(1))
                V_course_enh = self.xattn_norm_course(VNcourse + out_c.squeeze(1))
            else:
                raise ValueError(f"unknown fusion_mode={self.fusion_mode}")

            x_list = [V_user_enh, V_course_enh]

        elif multilevel_memory_data is not None and self.enable_multilevel_attention and len(x_list) >= 3:
            # ── Legacy: 旧版伪三级投影 (兼容) ──
            VNuser = x_list[0]
            VNcourse = x_list[1]
            VNanalysis = x_list[2]
            ...
            if self.enhanced_gating and 'enhanced_gating_features' in multilevel_memory_data:
                eg_feats = multilevel_memory_data['enhanced_gating_features']
                if eg_feats.dim() == 1:
                    eg_feats = eg_feats.unsqueeze(0).expand(VNuser.size(0), -1)
                gating_input = torch.cat([memory_ratio_tensor, eg_feats], dim=-1)
            else:
                gating_input = memory_ratio_tensor

            adaptive_weights = self.memory_weight_adaptor(gating_input)
            adaptive_weights = adaptive_weights.unsqueeze(-1)

            memory_stack = torch.stack([V_sensory, V_working, V_longterm], dim=1)
            V_memory = torch.sum(memory_stack * adaptive_weights, dim=1)

            V_user_enh, _ = self.sensory_attention(
                query=VNuser.unsqueeze(1), key=V_memory.unsqueeze(1), value=V_memory.unsqueeze(1)
            )
            V_user_enh = V_user_enh.squeeze(1)

            V_course_enh, _ = self.working_attention(
                query=VNcourse.unsqueeze(1), key=V_memory.unsqueeze(1), value=V_memory.unsqueeze(1)
            )
        ...
        # 2️⃣ MLP专家网络处理
        # analysis_as_expert: 用预提取的原始 analysis 向量投影（不依赖 x_list 是否还有第三项）
        analysis_proj_vec = None
        if self.analysis_as_expert and self.analysis_proj is not None and raw_analysis_vec is not None:
            analysis_proj_vec = self.analysis_proj(raw_analysis_vec)  # (B, inp_dim)
        ...
        for i, (gate_net, x) in enumerate(zip(self.gate_net, x_list)):
            ...
            if self.enhanced_gating:
                distribution = None if multilevel_memory_data is None else multilevel_memory_data.get(
                    'enhanced_gating_features'
                )
                if distribution is None:
                    distribution = x.new_zeros((x.size(0), 6))
                elif distribution.dim() == 1:
                    distribution = distribution.unsqueeze(0).expand(x.size(0), -1)
                gate_parts.append(distribution)
            if self.reflection_mode and self.transition_feature_dim > 0:
                transition = None if multilevel_memory_data is None else multilevel_memory_data.get(
                    'transition_features'
                )
                if transition is None:
                    transition = x.new_zeros((x.size(0), self.transition_feature_dim))
                elif transition.dim() == 1:
                    transition = transition.unsqueeze(0).expand(x.size(0), -1)
                gate_parts.append(transition)
            gate_input = torch.cat(gate_parts, dim=-1)
            gate_weights = gate_net(gate_input)  # (batch_size, total_expert_num)
            gates.append(gate_weights)
        ...
        for expert in self.share_expt_net:
            expert_outputs_per_task = []
            for x in x_list:
                expert_outputs_per_task.append(expert(x))
            share_expert_outputs.append(torch.stack(expert_outputs_per_task, dim=1))  # (B, task_num, E)

        # 任务特定MLP专家网络计算（user/course路径）
        spcf_expert_outputs = []
        for task_id, (task_experts, x) in enumerate(zip(self.spcf_expt_net, x_list)):
            task_expert_outputs = [expert(x) for expert in task_experts]
            spcf_expert_outputs.append(torch.stack(task_expert_outputs, dim=1))  # (B, spcf_num, E)

        # analysis 专家路径（可选）
        analysis_expert_outputs = []
        if analysis_proj_vec is not None and self.analysis_expt_net is not None:
            for task_id, task_experts in enumerate(self.analysis_expt_net):
                task_expert_outputs = [expert(analysis_proj_vec) for expert in task_experts]
                analysis_expert_outputs.append(torch.stack(task_expert_outputs, dim=1))  # (B, spcf_num, E)

        # 3️⃣ 专家输出融合
        task_outputs = []
        for task_id in range(self.task_num):
            gate_weights = gates[task_id]  # (B, total_expert_num)

            share_weights = gate_weights[:, :self._share_expt_num]
            rest_weights = gate_weights[:, self._share_expt_num:]
            spcf_weights = rest_weights[:, :self._spcf_expt_num]

            expert_output_dim = share_expert_outputs[0].size(-1) if share_expert_outputs else 64
            share_output = torch.zeros(x_list[task_id].size(0), expert_output_dim, device=x_list[task_id].device)
            for expert_id, expert_outputs in enumerate(share_expert_outputs):
                share_output += share_weights[:, expert_id:expert_id+1] * expert_outputs[:, task_id, :]

            spcf_output = torch.zeros_like(share_output)
            task_spcf_outputs = spcf_expert_outputs[task_id]
            for expert_id in range(task_spcf_outputs.size(1)):
                spcf_output += spcf_weights[:, expert_id:expert_id+1] * task_spcf_outputs[:, expert_id, :]

            # analysis 专家路径融合
            analysis_output = torch.zeros_like(share_output)
            if analysis_expert_outputs and task_id < len(analysis_expert_outputs):
                ana_weights = rest_weights[:, self._spcf_expt_num:]  # (B, spcf_num)
                task_ana_outputs = analysis_expert_outputs[task_id]
                for expert_id in range(task_ana_outputs.size(1)):
                    analysis_output += ana_weights[:, expert_id:expert_id+1] * task_ana_outputs[:, expert_id, :]

            task_output = share_output + spcf_output + analysis_output
            task_outputs.append(task_output)
...
class MultilevelMemoryMoE(nn.Module):
    """
    多级记忆混合专家网络 (Multilevel Memory Mixture of Experts)
    架构: 三级记忆多头注意力融合 → MLP专家网络 (单任务版本)
    """
    def __init__(self, moe_arch, inp_dim, dropout,
                 sensory_attn_heads=2, working_attn_heads=2, longterm_attn_heads=2,
        ...
        # 🧠 三级记忆多头注意力融合 (复用MultilevelMemoryHEA的逻辑)
        if self.enable_multilevel_attention:
            ...
            print(f"MultilevelMemoryMoE: 启用三级记忆多头注意力融合")
    ...
    def forward(self, x, multilevel_memory_data=None):
        ...
        Args:
            x: 输入向量 (batch_size, inp_dim)
            multilevel_memory_data: 多级记忆数据字典
        Returns:
            output: 专家网络输出 (batch_size, expert_arch[-1])
        """
        # 1️⃣ 三级记忆多头注意力融合
