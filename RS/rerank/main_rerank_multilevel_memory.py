...
from models import DLCM, EGRerank, GSF, LambdaRank, MIR, PEAR, PIER, PRM, RankFormer, SetRank
...
class MultilevelMemoryDataset(AmzDataset):
    def __init__(self, data_path, set="train", task="rerank", max_hist_len=10, augment=False, aug_prefix=None, memory_mode=True, enhanced_gating=False, reflection_mode=False, analysis_aug_file=None, no_analysis=False):
        super().__init__(data_path, set, task, max_hist_len, augment, aug_prefix, data_file=None)
...
def load_model_multilevel_memory(args, dataset):
    model_map = {
        ...
        "RankFormer": RankFormer,
        "PEAR": PEAR,
        "PIER": PIER,
    }
    if args.algo not in model_map:
        raise ValueError(f"Unsupported rerank model: {args.algo}")
...
def parse_args_multilevel_memory():
    ...
    parser.add_argument("--metric_scope", default="1,3,5,7,10")
    parser.add_argument("--task", default="rerank")
    parser.add_argument("--algo", default="DLCM")
