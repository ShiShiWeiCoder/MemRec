...
def evaluate_rerank(labels, preds, scope_number, is_rank, compute_auc=False):
    ...
    for label, pred in zip(labels, preds):
        ...
        for i, scope in enumerate(scope_number):
            ...
            relevant_count = sum(1 for value in label if value >= 1)
            ...
            ap_denominator = min(relevant_count, cur_scope)
            _map = float(AP_value) / ap_denominator if ap_denominator else 0.
...
def save_paper_metrics(path, task, algorithm, metric_scope, results, seed=None):
    """Persist the exact metric set reported in the paper as machine-readable JSON."""
    report = {
        "task": task,
        "algorithm": algorithm,
        "seed": seed,
        "metrics": {
            f"MAP@{scope}": float(results[0][index])
            for index, scope in enumerate(metric_scope)
        },
    }
    report["metrics"].update({
        f"NDCG@{scope}": float(results[1][index])
        for index, scope in enumerate(metric_scope)
    })
    report["metrics"].update({
        f"HR@{scope}": float(results[2][index])
        for index, scope in enumerate(metric_scope)
    })
    report["metrics"]["MRR"] = float(results[3])
    if results[4] is not None:
        report["metrics"]["AUC"] = float(results[4])
    os.makedirs(os.path.dirname(os.path.abspath(path)), exist_ok=True)
    with open(path, "w", encoding="utf-8") as handle:
        json.dump(report, handle, indent=2, sort_keys=True)
    print(f"Paper metrics JSON: {path}")
    return report
